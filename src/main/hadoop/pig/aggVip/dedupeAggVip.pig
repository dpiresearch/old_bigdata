...
--    This pig script dedups counts (aggVip) between two dates.
--
--    Input file:
...
EVT_NOW = LOAD '$inputdir/$thedate/aggvip/part*' as (adid:long, usrid:long, count:long);
EVT_BEFORE = LOAD '$inputdir/$olddate/aggvip/part*' as (adid:long, usrid:long, count:long);
...
STORE RES INTO '$outputdir/$thedate/aggDeltaVip';

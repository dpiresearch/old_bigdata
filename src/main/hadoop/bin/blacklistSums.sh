...
echo "Calculating for $thedate"

# TODO - check for existence, otherwise it'll throw an error
echo hadoop fs -rmr $outputdir/$thedate/clean
hadoop fs -rmr $outputdir/$thedate/clean
echo hadoop fs -rmr $outputdir/$thedate/blacklisted 
hadoop fs -rmr $outputdir/$thedate/blacklisted 

echo pig -param inputdir=$inputdir -param thedate=$thedate -f $deploydir/pig/blacklistSums.pig
pig -param inputdir=$inputdir -param thedate=$thedate -param deploydir=$deploydir -param outputdir=$outputdir -f $deploydir/pig/blacklistSums.pig

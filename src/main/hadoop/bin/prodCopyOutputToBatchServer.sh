#!/bin/bash
# prodCopyOutputToBatchServer.sh
#
# Generic script which scp's hadoop output files from
# hdfs into Batch pool servers.
#
# Currently being used by bestads, vipvisits, phoneclick count jobs.
#
# set defaults

DAYS_AGO=0
INPUT_DIR=/user/dapang/data/prod/viewad
...
echo "scp /tmp/$filename ${username}@batch001:$outputdir"
scp /tmp/$filename ${username}@batch001:$outputdir

echo "/tmp/$filename ${username}@batch002:$outputdir"
scp /tmp/$filename ${username}@batch002:$outputdir

# echo "/tmp/$filename ${username}@batch003:$outputdir"
# scp /tmp/$filename ${username}@batch003:$outputdir
